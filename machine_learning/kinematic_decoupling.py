"""
Model 2: Pure Kinematic Decoupling (The Lying Vector Attack)
Description: The agent falsifies both position and velocity kinematics. The environment 
simulates a receiving node's EKF calculating the NIS using an anisotropic covariance 
matrix. The agent must learn to align its drift with the axis of highest uncertainty.
"""
...
class KinematicDecouplingEnv(gym.Env):
    def __init__(self):
        super().__init__()
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(6,), dtype=np.float32)
        self.observation_space = spaces.Box(low=-20000.0, high=20000.0, shape=(12,), dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.network_nis_dict = {self.attacking_node_id: 5.348}
        self.cumulative_r_offset = np.zeros(3, dtype=np.float32)
        self.cumulative_v_offset = np.zeros(3, dtype=np.float32)
        ...
        for i in range(4):
            ...
            nominal_tx = self._build_transaction(warmup_r_vector, warmup_v_vector, 5.348)
        ...
        self.current_state = np.concatenate([self.nominal_r_vector, self.nominal_v_vector, self.cumulative_r_offset, self.cumulative_v_offset], dtype=np.float32)
    ...
    def _build_transaction(self, r_vector, v_vector, step_nis):
        ...
        obs_record = ObservationRecord(
            ...
            r_vector=list(r_vector), v_vector=list(v_vector), nis=step_nis, dof=6
        )
        tx_data_json = json.dumps(dataclasses.asdict(obs_record))
        metadata = TransactionMetadata()
        metadata.observer_id = self.attacking_node_id
        addresses = TransactionAddresses(sender_address=hash(self.attacking_node_id), recipient_address=self.target_node_id, sender_private_key="PLACEHOLDER_KEY")
    ...
    def step(self, action):
        ...
        self.nominal_r_vector = np.array([radius * math.cos(angle), radius * math.sin(angle), radius * 0.1 * math.sin(angle)])
        self.nominal_v_vector = np.array([-radius * orbital_rate * math.sin(angle), radius * orbital_rate * math.cos(angle), radius * 0.1 * orbital_rate * math.cos(angle)])

        scaled_action_r = action[0:3] * 15.0
        scaled_action_v = action[3:6] * 0.05
        
        self.cumulative_r_offset += scaled_action_r
        self.cumulative_v_offset += scaled_action_v

        malicious_r_vector = self.nominal_r_vector + self.cumulative_r_offset
        malicious_v_vector = self.nominal_v_vector + self.cumulative_v_offset

        y_r = malicious_r_vector - self.nominal_r_vector
        y_v = malicious_v_vector - self.nominal_v_vector
        y = np.concatenate([y_r, y_v])

        S_matrix = np.diag([150.0, 10.0, 10.0, 1.0, 0.1, 0.1])
        S_inv = np.linalg.inv(S_matrix)
        calculated_nis = float(y.T @ S_inv @ y)

        mock_transaction = self._build_transaction(malicious_r_vector, malicious_v_vector, calculated_nis)
        ...
        consensus_state = self.attacking_satellite_node.dag.local_consensus_states.get(mock_transaction.hash, {})
        ...
        self.current_state = np.concatenate([malicious_r_vector, malicious_v_vector, self.cumulative_r_offset, self.cumulative_v_offset], dtype=np.float32)
        
        return self.current_state, reward, False, self.current_step >= self.max_steps, {}

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s",
                        handlers=[logging.FileHandler("kinematic_attack.log", mode="w", encoding="utf-8"), logging.StreamHandler()])
    logging.getLogger("src").setLevel(logging.WARNING)
    logger = logging.getLogger("PoISELogger")

    env = KinematicDecouplingEnv()
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.001)
    
    logger.info("Training Kinematic Decoupling Model...")
    model.learn(total_timesteps=300000)
    model.save("kinematic_decoupling_injector")

    logger.info("Executing test episode.")
    obs, info = env.reset()
    for i in range(360):
        action, _ = model.predict(obs, deterministic=True)
        obs, reward, terminated, truncated, _ = env.step(action)
        logger.info(f"Step {i+1} | Action: {np.round(action, 4)} | Reward: {reward:.4f} | Pos Drift: {np.linalg.norm(env.cumulative_r_offset):.2f} km | Vel Drift: {np.linalg.norm(env.cumulative_v_offset):.4f} km/s")
        if truncated: break
    ...
    ax.plot(history["attack_x"], history["attack_y"], history["attack_z"], label='Kinematic Spoofed Track', color='red', marker='o', markersize=3)
    ax.set_title('AI-Driven Kinematic Decoupling Attack')
    ax.legend()
    plt.show()
