...
This research introduces the Autonomous Cooperative Consensus Orbit Determination (ACCORD) framework, a decentralised approach to satellite operations in Low Earth Orbit (LEO). To overcome the limitations of centralised coordination, including vulnerability to single points of failure and trust barriers, ACCORD employs a Directed Acyclic Graph (DAG)-based Distributed Ledger Technology (DLT). Satellites collaboratively determine each other’s orbits through mutual observation using on-board sensors, and submit this data to the ledger. A reputation-based, Byzantine Fault Tolerant consensus mechanism, validates these observations without relying on financial incentives or intermediaries.
